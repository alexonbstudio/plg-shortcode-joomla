...
if(!function_exists('whois_lookup_sc')) {
#######################################################################
...
if(substr(strtolower($domain), 0, 7) == "https://") $domain = substr($domain, 7); // remove https:// if included
...
if(!$server) {
return "Erreur: Aucun serveur Whois appropriées correspondent à $domain du nom de domaine.";
}
...
return $out;
	...
	function whois_lookup_sc( $atts, $content) {
		extract(bbcodes_atts(array(
				'url' => ''
		 ), $atts));
		 
		$links = ($url !='') ? $url : '127.0.0.1';
$whois=new Whois;
$rs=$whois->whoislookup($links); //Your domain or IP
	
		
		return '<pre>'.$rs.'</pre>'.$content;
	}
		
	add_bbcodes( 'whois-domains', 'whois_lookup_sc' );
}